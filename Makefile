...
KEY     := server_key.pem
INVSH   := ./scripts/update-inventory.sh
...
# The script is runnable on its own when only the address has changed.
inventory:
	@$(INVSH)
...
# Destroys the instance and the whole network around it. The Elastic IP is
# not in this state - see terraform/main.tf - so the address survives and the
# domain's A records stay valid for the next `make up`. AWS bills an allocated
# IPv4 address even while nothing is attached to it: about $3.60 a month. Run
# `aws ec2 release-address --allocation-id ...` to give it back for good.
down:
