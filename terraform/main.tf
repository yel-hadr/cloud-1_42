...
resource "aws_key_pair" "deployer" {
  ...
  # AWS stores the public key with whatever comment it was uploaded with,
  # while server_key.pub is regenerated by the Makefile from the private
  # half. When the two comments differ the material is identical but
  # Terraform still sees a diff on `public_key`, which forces replacement of
  # the key pair on an otherwise no-op plan. Ignoring the attribute keeps
  # those plans clean. To genuinely rotate the key, replace it explicitly:
  #   terraform -chdir=terraform apply -replace=aws_key_pair.deployer
  lifecycle {
